...
export function CartDrawer({ items, onChangeQuantity, onSubmit }: CartDrawerProps) {
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} aria-label={`Відкрити кошик${quantity ? `, ${quantity} товарів на ${formatPrice(total)}` : ""}`} className="fixed bottom-4 right-4 z-30 flex items-center gap-2 rounded-2xl border-2 border-emerald-300 bg-emerald-700 px-4 py-3 text-sm font-black text-white shadow-2xl transition hover:bg-emerald-800 focus:outline-none focus:ring-4 focus:ring-emerald-200 sm:bottom-6 sm:right-6 sm:text-base">
      <span aria-hidden="true">🛒</span><span>Кошик</span>{quantity > 0 && <><span className="inline-flex min-w-6 items-center justify-center rounded-full bg-white px-2 py-0.5 text-sm text-emerald-900">{quantity}</span><span className="hidden border-l border-emerald-400 pl-2 text-xs text-emerald-50 sm:inline">{formatPrice(total)}</span></>}
    </button>
